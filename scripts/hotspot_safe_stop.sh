#!/bin/bash
# Safety stop: stop AP/DHCP/nginx only — does NOT run undo_hotspot_stack (no NetworkManager
# restart). Same skip file as Disable so a dgx-app restart does not immediately start the stack
# again (configured marker may still exist). /tmp skip clears on reboot.
set -e
SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
SKIP_FLAG=/tmp/dgx-hotspot-skip-autostart
HOTSPOT_USER="${DG_HOTSPOT_USER:-echomind}"

echo "==> Suppressing hotspot autostart until Enable or reboot..."
touch "$SKIP_FLAG"
