...
def _run_compose_restart() -> tuple[bool, str, str]:
    ...
    return True, ECHOMIND_PUBLIC_URL, "EchoMind stack is up."
...
def _run_personaplex_conversation() -> tuple[bool, str]:
    ...
    r = subprocess.run(
        ...
        capture_output=True,
        text=True,
        timeout=int(os.environ.get("PERSONAPLEX_BUILD_TIMEOUT_SEC", "1800")),
        env=os.environ.copy(),
    )
    combined = (r.stdout or "") + (r.stderr or "")
    if r.returncode != 0:
...
async def echomind_start():
    ok, url, message = await asyncio.to_thread(_run_compose_restart)
    if not ok:
        raise HTTPException(status_code=500, detail=message)
    return EchomindStartResponse(ok=True, url=url, message=message)
